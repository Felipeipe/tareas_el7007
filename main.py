...
def main():
    ...
    for alpha in ALPHA:
        ...
        plot_img(os.path.join(OUT_PART_1, f"perro_{alpha}.pdf"), watermark_img, title=f"Peak Signal-to-Noise Ratio = {current_psnr:.3f}dB")
    ...
    bars_true = plt.bar(x - 1.5 * width, all_true_corrs, width, label='Clave Original', color='#2ca02c', edgecolor='black')
    bars_f1 = plt.bar(x - 0.5 * width, all_fake_corrs[0], width, label='Clave Falsa 1', color='#ff7f0e', edgecolor='black')
    bars_f2 = plt.bar(x + 0.5 * width, all_fake_corrs[1], width, label='Clave Falsa 2', color='#1f77b4', edgecolor='black')
    bars_f3 = plt.bar(x + 1.5 * width, all_fake_corrs[2], width, label='Clave Falsa 3', color='#d62728', edgecolor='black')
    ...
    plt.title("Correlación entre distintas llaves, con distintos valores de alpha", fontsize=14, fontweight='bold')
